...
def folder_initialization(date: str, seed: int, single_point_methods_names: list, refiners: list, scalarization: bool, NS_local_opt: str):
            ...
            for single_point_method_name in single_point_methods_names:
                for refiner in refiners:
                    if scalarization:
                        ...
                        postfix = '-' + str(NS_local_opt)
                    else:
                        postfix = ''
                    make_folder(os.path.join(path, folder, '{}-{}'.format(single_point_method_name + postfix, refiner)))


def execution_time_file_initialization(date: str, seed: int, single_point_methods_names: list, refiners: list, scalarization: bool, NS_local_opt: str):
    for single_point_method_name in single_point_methods_names:
        for refiner in refiners:
            if scalarization:
                postfix = '-Scal'
                ...
                postfix = ''
            execution_time_file = open(os.path.join('Execution_Outputs', date, str(seed), 'Execution_Times', '{}-{}.txt'.format(single_point_method_name + postfix, refiner)), 'w')
